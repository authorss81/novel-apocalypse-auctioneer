...
**Paragraph shape, and the denominator is fixed here because three readings of "non-quote paragraph" were in circulation in the last batch and each was quoted as though it were the only one.** A *narration paragraph* is a body paragraph that is not a blockquote, is not wholly bold, and does not open with a quotation mark; a paragraph counts as single-sentence if splitting it on sentence-ending punctuation followed by a space returns one segment. **Chapters 101 to 110 on that definition run 59.1, 65.2, 65.1, 62.8, 76.9, 63.0, 70.8, 79.5, 80.0 and 80.4 per cent, which is 322 of 457 and seventy point five in a hundred**, against **51.3 in Batch 0005 measured the same way**, so this batch is about nineteen points higher and the comparison is like for like. The one-sentence documentary register is this book's voice and is not to be sanded off, and **the shape rises across that batch because its last four chapters tell a clerk's ledger, a month turning, a toll board and a page in a public book, which are told in one-sentence lines.** **Two other denominators are recorded so that a check reproduces whichever one it runs: excluding blockquotes only, 396 of 759 and fifty-two point two; excluding blockquotes and headers, 391 of 707 and fifty-five point three.** The series 52.2, 65.7, 74.0, 68.1, 68.6, 70.8, 80.6, 80.0, 80.0, 86.1 and 508 of 697 belongs to none of the three and is withdrawn. **The longest single-sentence narration paragraph in Chapters 101 to 110 is 124 words, in Chapter 110, and nothing in that measure passes a hundred and thirty** — break at the last clean conjunction and leave a paragraph break rather than a rewrite. **Seven paragraphs in that batch are longer than a hundred and words, and all seven are headers or blockquotes or multi-sentence narration: three in Chapter 101 at 133, 146 and 140, two blockquotes in Chapter 103 at 172 and 157, and two headers in Chapter 105 at 179 and 170.** A one hundred and twenty-eight word paragraph in Chapter 106 is a wholly bold header and is not a single-sentence paragraph; do not quote it as one.

**Tics, counted by word boundary and reproducible, over the body with the title line excluded. _In the yard_ 12 and _in a yard_ 22, against 6 and 14 in Batch 0005. _At the rail_ 2 and _on the rail_ 0. _About nine_ 36 and _about eleven_ 24, together 60 against 37 in Batch 0005 — this is the figure that went the wrong way last batch and has gone further the wrong way, and it is disclosed rather than explained. _About_ plus a number word is 303 and _about_ plus any word is 511, against 202 and 370 in Batch 0005; the two methods are not like for each other because the prose writes its numbers as words, and the _about_ counts are case-sensitive, since a case-insensitive count returns 520. Thirty hedges were moved on the writing pass and the remainder is the district's own arithmetic. _I would like_ 30, and every one of the thirty is in a mouth, which was checked by pairing the quotation marks in each chapter and not by eye. _held_ 7 and is structural. _steady_ 0, _breathed_ 0, _nodded_ 0, _literacy_ 0.**
...
# The two standing invariants the prose asserts in words. The loop above is
# scoped to your own chapters, so nothing above would catch a change in them.
for vol,expect in (('volume-01',10),('volume-02',7),('volume-03',None)):
    tot=0; locs=[]
    for f in sorted(glob.glob(f'chapters/{vol}/*.md')):
        n=int(re.search(r'chapter-(\d+)',f).group(1))
        if vol=='volume-03' and 101<=n<=110: continue
        raw=open(f).read()
        odd=sum(1 for p in raw.split('\n\n') if p.count('**')%2)
        if not odd: continue
        tot+=odd
        # the carried-defect list is cited by LINE, the invariant is counted by
        # PARAGRAPH; report the lines so the two can be compared directly.
...
**Read that second loop as a guard, not as a target.** Volume 02 is **exactly seven** and must still be exactly seven; Chapters 101 to 110 are **zero** and must stay zero; **Volume 01 holds ten**, in Chapters 16, 40, 42, 43 and 45, and that figure is carried, canon and closed. **An unscoped run over the whole manuscript therefore returns seventeen and not seven, and a reviewer will ask about it, so state the scope whenever you quote the number.** The Volume 01 ten and the Volume 02 seven are not to be repaired by a writer phase.

**Seven defects from earlier batches are carried, unfixed, and are outside the current phase,** and the seven are the seven the odd-paragraph count returns: `chapter-0058.md:23`, `chapter-0064.md:85`, `chapter-0066.md:100`, `chapter-0066.md:102`, `chapter-0067.md:137`, `chapter-0067.md:145`, `chapter-0068.md:85`. **A previous version of this list also named `chapter-0070.md:15` and `chapter-0070.md:23`; both are void, because that chapter has no odd-marker line and no odd-marker paragraph, and the two references have been removed rather than renumbered.** **Chapters 51 to 100 are canon and are not to be rewritten for this. Do not add an eighth.** Run the check scoped to your own chapters so the seven carried hits do not read as new failures, and note that **the count of odd paragraphs in the whole of Volume 02 is exactly seven and must still be exactly seven, and that the count in Chapters 101 to 110 is zero and must stay zero.**

**One repeated line in the last batch is deliberate and must not be cut as a duplicated paragraph.** The blockquote **I DO NOT LIKE IT, AND I SAY YES.** stands at `chapter-0105.md:61` and again at `chapter-0106.md:27`: it is the same woman's standing answer, given in the same order every month since the twenty-ninth of March, and the second chapter says so in the paragraph that follows it. If you repeat a standing answer for the same reason, say so in the prose, and record it here.
